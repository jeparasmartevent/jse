...
class Model_customer extends CI_Model {
	...
	public function get_shopping_history($user)
		...
		public function mark_invoice_paid_confirmed($input)
		{//check the invoice


			$this->db->where('id', $input['id']);
		    $query=$this->db->update('invoices', $input);
		    return $query;
			// $ret = TRUE ;
			// $is_invoice = $this->db->where('id',$input['invoice_id_input'])->limit(1)->get('invoices');
			// if($is_invoice->num_rows() == 0  )
			// {
			// 		$ret = $ret && FALSE;
			// }else{//check the amount
			// 		$total = $this->db->select('SUM(qty * price) AS total')
			// 						  ->where('invoice_id',$input['invoice_id_input'])
			// 						  ->get('orders');
			...
			// 				$this->db->where('id',$input['invoice_id_input'])->update('invoices',array('status'=>'Menunggu_Konfirmasi'));
			// 			}
